...
from src.config.settings import AppSettings, _simple_encrypt, _simple_decrypt
...
class TestProviderConfigDecryption:
    ...
    def test_save_load_decrypts_current_provider_key(self, temp_config_file):
        ...
        settings.config.set_provider_config("deepseek", ProviderConfig(
            api_key="deepseek-key-123",
            base_url="https://api.deepseek.com/v1",
            model="deepseek-chat"
        ))
        settings.config.set_provider_config("openrouter", ProviderConfig(
            api_key="openrouter-key-456",
            base_url="https://openrouter.ai/api/v1",
            model="deepseek/deepseek-chat"
        ))
        settings.config.set_provider_config("openai-format", ProviderConfig(
        ...
        # 验证所有供应商的密钥都已解密
        assert settings2.config.ai_providers_config["deepseek"].api_key == "deepseek-key-123", \
            f"deepseek 期望 deepseek-key-123，实际: {settings2.config.ai_providers_config['deepseek'].api_key}"
        assert settings2.config.ai_providers_config["openrouter"].api_key == "openrouter-key-456", \
            f"openrouter 期望 openrouter-key-456，实际: {settings2.config.ai_providers_config['openrouter'].api_key}"
        assert settings2.config.ai_providers_config["openai-format"].api_key == "openai-key-789", \
    ...
    def test_all_provider_keys_encrypted_in_saved_file(self, temp_config_file):
        ...
        # 加密后的密钥不应该是明文
        assert saved_data["ai_providers_config"]["deepseek"]["api_key"] == _simple_encrypt("deepseek-key")
        assert saved_data["ai_providers_config"]["openrouter"]["api_key"] == _simple_encrypt("openrouter-key")
        assert saved_data["ai_providers_config"]["openai-format"]["api_key"] == _simple_encrypt("openai-key")
    ...
    def test_switch_provider_after_load(self, temp_config_file):
        ...
        # 验证 deepseek 已解密
        assert settings.config.ai_providers_config["deepseek"].api_key == "deepseek-secret", \
        ...
        assert openrouter_config.api_key == "openrouter-secret", \
