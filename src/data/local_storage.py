...
class LocalStorage:
    ...
    def clear_old_sessions(self, days: int = 30) -> int:
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(f"清理 {deleted_count} 个旧会话（超过 {days} 天）")
                ...
                "sessions": [
                    {
                        "id": s.id,
                        "problem": s.problem,
                        "improving_param": s.improving_param,
                        "worsening_param": s.worsening_param,
        ...
        try:
            if format.lower() == "json":
                ...
                lines = [
                    ...
                    f"矩阵类型: {session.matrix_type}矛盾矩阵",
                    f"改善参数: {session.improving_param or '未指定'}",
